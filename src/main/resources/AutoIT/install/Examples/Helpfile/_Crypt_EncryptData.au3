		...
		Switch GUIGetMsg()
			...
			Case $idCombo ; Check when the combobox is selected and retrieve the correct algorithm.
				...
				If StringStripWS($sRead, $STR_STRIPALL) <> "" Then ; Check there is text available to encrypt.
					...
					GUICtrlSetData($g_idEdt_Output, $dEncrypted) ; Set the output box with the encrypted text.
					Local $dDecrypted = _Crypt_DecryptData($dEncrypted, $g_hKey, $CALG_USERKEY) ; Decrypt the text with the new cryptographic key.
					GUICtrlSetData($g_idEdt_OutputDeCrypted, BinaryToString($dDecrypted)) ; Set the output box with the encrypted text.
				EndIf
		EndSwitch
	WEnd
	...
	Switch _WinAPI_LoWord($wParam)
		Case $g_idEdt_Input
			Switch _WinAPI_HiWord($wParam)
				Case $EN_CHANGE
					Local $dEncrypted = _Crypt_EncryptData(GUICtrlRead($g_idEdt_Input), $g_hKey, $CALG_USERKEY) ; Encrypt the text with the cryptographic key.
					GUICtrlSetData($g_idEdt_Output, $dEncrypted) ; Set the output box with the encrypted text.
					Local $dDecrypted = _Crypt_DecryptData($dEncrypted, $g_hKey, $CALG_USERKEY) ; Decrypt the text with the new cryptographic key.
					GUICtrlSetData($g_idEdt_OutputDeCrypted, BinaryToString($dDecrypted)) ; Set the output box with the encrypted text.
			EndSwitch
