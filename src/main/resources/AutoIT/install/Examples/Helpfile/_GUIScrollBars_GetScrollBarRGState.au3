...
Func Example()
	...
	_MemoWrite("Horizontal (Before)" & @CRLF & "--------------------------------------")
	For $x = 0 To 5
		_MemoWrite("rgstate[" & $x & "]...: " & $aRGState[$x])
	Next

	_MemoWrite(@CRLF & "Disable both arrows: " & _GUIScrollBars_EnableScrollBar($hGUI, $SB_HORZ, $ESB_DISABLE_BOTH) & @CRLF)

	$aRGState = _GUIScrollBars_GetScrollBarRGState($hGUI, $OBJID_HSCROLL)
	_MemoWrite("Horizontal (After)" & @CRLF & "--------------------------------------")
	For $x = 0 To 5
		_MemoWrite("rgstate[" & $x & "]...: " & $aRGState[$x])
	Next
