		...
		Switch GUIGetMsg()
			...
			Case $idCombo ; Check when the combobox is selected and retrieve the correct algorithm.
				...
				If StringStripWS($sRead, $STR_STRIPALL) <> "" Then ; Check there is text available to hash.
					...
					GUICtrlSetData($g_idEdt_Output, $dHash) ; Set the output box with the hash data.
				EndIf
		EndSwitch
	WEnd
	...
	Switch _WinAPI_LoWord($wParam)
		Case $g_idEdt_Input
			Switch _WinAPI_HiWord($wParam)
				Case $EN_CHANGE
					Local $dHash = _Crypt_HashData(GUICtrlRead($g_idEdt_Input), $g_iAlgorithm) ; Create a hash of the text entered.
					GUICtrlSetData($g_idEdt_Output, $dHash) ; Set the output box with the hash data.
			EndSwitch
	EndSwitch
EndFunc   ;==>WM_COMMAND
