...
;~ 	_GUICtrlTreeView_SetUnicodeFormat($idTreeView, False)
	...
	For $x = 0 To $iRand
		...
		For $y = 0 To $iChildRand
			$aidTVi_Child[$iYItem] = GUICtrlCreateTreeViewItem(StringFormat("[%02d] New Item", $y), $aidTVi_Item[$x])
			$iYItem += 1
		Next
	Next
	...
	$iRand = 5 ;Random(0, 9)
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Sibling Count for item index %d: %d", $iRand, _GUICtrlTreeView_GetSiblingCount($idTreeView, $aidTVi_Item[$iRand])))

	$iRand = 7 ; Random(0, 12)
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Sibling Count for Child item index %d: %d", $iRand, _GUICtrlTreeView_GetSiblingCount($idTreeView, $aidTVi_Child[$iRand])))
